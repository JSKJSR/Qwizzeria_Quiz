...
const NAV_ICONS = {
  ...
  doubles: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>,
  ...
  pricing: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><line x1="12" y1="1" x2="12" y2="23" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>,
  ai: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /><path d="M2 12l10 5 10-5" /></svg>,
};

const LockIcon = () => (
  <svg className="dashboard__nav-lock" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default function DashboardLayout() {
  const { isEditor, signOut, hasTier, subscription, isTrial, isGated } = useAuth();
    ...
    <div className="dashboard">
      ...
      <aside className={`dashboard__sidebar ${sidebarOpen ? 'dashboard__sidebar--open' : ''}`}>
        ...
        <nav className="dashboard__nav">
          {NAV_ITEMS.map((item) => {
            const locked = item.requiredTier && !hasTier(item.requiredTier);
            return (
              <NavLink
                key={item.to}
                to={item.to}
          ...
          {(isGated || isTrial) && subscription.status !== 'staff' && (
            <NavLink
              to="/pricing"
          ...
          <div className="dashboard__nav-link dashboard__nav-link--disabled">
            <div className="dashboard__nav-link-content">
              <span className="dashboard__nav-icon">{NAV_ICONS.ai}</span>
              Generate with AI
            </div>
            <span className="dashboard__nav-badge">Coming Soon</span>
          </div>
          {isEditor && (
            <NavLink
